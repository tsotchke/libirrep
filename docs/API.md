...
8. **Softest-mode locator**: `irrep_magnon_softest_mode(L, Nx, Ny,
   ...
   points (when bands touch), flat bands (constant minimum over a
   region), and Goldstone-mode locations. Verified: square FM
   minimum at Γ exactly; with exclude_below filter, next-softest
   at finite k.
8. **Spin gap + bandwidth**: `irrep_magnon_band_extrema(L, Nx, Ny,
