...
| R3 | 3D Bombín gauge color code | Bombín 2010 PRA 81 032301 | High | ✅ Shipped: `<irrep/color_code_3d.h>` ([[8,3,2]] d=2 cubic + [[15,1,3]] d=3 Reed-Muller/tetrahedral, both brute-force-distance verified) + `<irrep/color_lattice_3d.h>` (generic 3-colex face-and-cell-list builder; tetrahedral instance subsumes the direct construction and serves as the framework's correctness witness) + `irrep_subsystem_bombin_3d_tetrahedron` (gauge substrate). Arbitrary-L rectified-3-cubic instances are now reducible to face/cell-list tabulation against the framework |
| R4 | Multi-tile HaPPY hyperbolic network | Pastawski-Yoshida-Harlow-Preskill 2015 JHEP 06 149 | High | ✅ Shipped: `<irrep/happy_network.h>` — `irrep_happy_perfect_tensor_6leg` (single tile) + `irrep_happy_network_depth2` (36-qubit joined group + contraction pairs) + `irrep_happy_network_depth2_contracted` (5-edge Bell-pair contraction → 26-qubit [[20,6,?]] encoding-isometry stabilizer group with bulk indices `{0, 1, 6, 11, 16, 21}`) |
