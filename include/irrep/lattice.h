...
IRREP_API irrep_lattice_t *irrep_lattice_build(irrep_lattice_kind_t kind, int Lx, int Ly);
...
IRREP_API void irrep_lattice_primitive_vectors(const irrep_lattice_t *L, double a1[2],
                                               double a2[2]);

/** @brief Write reciprocal primitive vectors @p b1, @p b2 satisfying
 *         `a_i · b_j = 2π δ_{ij}`. */
IRREP_API void irrep_lattice_reciprocal_vectors(const irrep_lattice_t *L, double b1[2],
                                                double b2[2]);

/** @brief Cartesian position of @p site. Returns #IRREP_OK or
 *         #IRREP_ERR_INVALID_ARG. */
IRREP_API irrep_status_t irrep_lattice_site_position(const irrep_lattice_t *L, int site,
                                                     double xy[2]);
 ...
 *         #IRREP_ERR_INVALID_ARG. */
IRREP_API irrep_status_t irrep_lattice_cell_of(const irrep_lattice_t *L, int site, int *ix,
                                               int *iy);

/** @brief Compose a site index from `(ix, iy, sublattice)`. PBC is applied
 *         to `ix, iy`; returns `-1` on out-of-range sublattice. */
IRREP_API int irrep_lattice_site_index(const irrep_lattice_t *L, int ix, int iy, int sublattice);
/** @} */
...
IRREP_API int irrep_lattice_translate(const irrep_lattice_t *L, int site, int dx, int dy);
/** @} */
...
IRREP_API void irrep_lattice_fill_bonds_nn(const irrep_lattice_t *L, int *i_out, int *j_out);

/** @brief Write the NNN bond list; semantics match @ref irrep_lattice_fill_bonds_nn. */
IRREP_API void irrep_lattice_fill_bonds_nnn(const irrep_lattice_t *L, int *i_out, int *j_out);
/** @} */
...
IRREP_API void irrep_lattice_k_grid(const irrep_lattice_t *L, double *kx, double *ky);
/** @} */
