...
irrep_linear_t *irrep_linear_build(const irrep_multiset_t *in, const irrep_multiset_t *out,
    ...
    for (int ti = 0; ti < in->num_terms; ++ti) {
        for (int to = 0; to < out->num_terms; ++to) {
            if (in->labels[ti].l != out->labels[to].l)
                continue;
            if (in->labels[ti].parity != out->labels[to].parity)
                continue;
...
void irrep_linear_apply(const irrep_linear_t *lin, const double *weights, const double *in,
                        double *out) {
    if (!lin)
        return;
    for (int i = 0; i < lin->out_dim; ++i)
        out[i] = 0.0;
...
void irrep_linear_backward(const irrep_linear_t *lin, const double *weights, const double *in,
                           const double *grad_out, double *grad_weights, double *grad_in) {
    if (!lin)
        return;
    for (int k = 0; k < lin->num_recipes; ++k) {
        const struct linear_recipe *r = &lin->recipes[k];
...
void irrep_norm_rms(const irrep_multiset_t *m, int channels, const double *scales, const double *in,
                    double *out) {
    UNUSED(channels);
    if (!m)
        return;
...
void irrep_norm_rms_backward(const irrep_multiset_t *m, int channels, const double *scales,
                             const double *in, const double *grad_out, double *grad_scales,
                             double *grad_in) {
    UNUSED(channels);
    if (!m)
        return;
...
void irrep_gate_apply(const irrep_multiset_t *m, int channels, const double *scalar_gates,
