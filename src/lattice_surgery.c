/* SPDX-License-Identifier: MIT */
/** @file lattice_surgery.c
 *  @brief Implementation of smooth- and rough-merge lattice surgery on
 *         rotated surface codes. Both merged geometries are rectangular
 *         rotated surface codes — `d × 2d` for smooth (patches abutted
 *         along smooth/Z-boundary) or `2d × d` for rough (patches
 *         abutted along rough/X-boundary). The stabilizer rules
 *         generalise those of `<irrep/surface_code.h>` from square
 *         `d × d` to rectangular `n_rows × n_cols` without changing
 *         the parity assignment. */
#include <irrep/lattice_surgery.h>
#include <irrep/types.h>

#include <stddef.h>

static inline int qcoord(int r, int c, int n_cols) { return r * n_cols + c; }

/* ====================================================================
 * Rectangular surface-code stabilizer counts (n_rows × n_cols).
 *
 * Same alternation rule as `<irrep/surface_code.h>`: bulk plaquette
 * at (a, b) is X-type if a+b even, Z-type if odd. Boundary stabs:
 *   - top X (r=0, b odd)
 *   - bottom X (r=n_rows-1, (n_rows-1+b) even)
 *   - left Z (c=0, a even)
 *   - right Z (c=n_cols-1, (a+n_cols-1) odd)
 * ==================================================================== */
static int count_bulk_X(int n_rows, int n_cols) {
    int n = 0;
    for (int a = 0; a < n_rows - 1; ++a)
        for (int b = 0; b < n_cols - 1; ++b)
            if (((a + b) & 1) == 0) ++n;
    return n;
}
static int count_bulk_Z(int n_rows, int n_cols) {
    return (n_rows - 1) * (n_cols - 1) - count_bulk_X(n_rows, n_cols);
}
static int count_top_X(int n_cols) {
    int n = 0;
    for (int b = 0; b < n_cols - 1; ++b) if ((b & 1) == 1) ++n;
    return n;
}
static int count_bottom_X(int n_rows, int n_cols) {
    int n = 0;
    for (int b = 0; b < n_cols - 1; ++b)
        if (((n_rows - 1 + b) & 1) == 0) ++n;
    return n;
}
static int count_left_Z(int n_rows) {
    int n = 0;
    for (int a = 0; a < n_rows - 1; ++a) if ((a & 1) == 0) ++n;
    return n;
}
static int count_right_Z(int n_rows, int n_cols) {
    int n = 0;
    for (int a = 0; a < n_rows - 1; ++a)
        if (((a + n_cols - 1) & 1) == 1) ++n;
    return n;
}

/* Shared rectangle builder. Emits H_X (bulk X then top X then bottom X)
 * and H_Z (bulk Z then left Z then right Z) into `out`, which must
 * already be allocated to the correct (n_X_stabs, n_Z_stabs) shape. */
static irrep_status_t
build_rect_surface_code(irrep_css_code_t *out, int n_rows, int n_cols)
{
    int xrow = 0;
    for (int a = 0; a < n_rows - 1; ++a) {
        for (int b = 0; b < n_cols - 1; ++b) {
            if (((a + b) & 1) == 0) {
                irrep_parity_matrix_set(&out->H_X, xrow, qcoord(a,     b,     n_cols));
                irrep_parity_matrix_set(&out->H_X, xrow, qcoord(a + 1, b,     n_cols));
                irrep_parity_matrix_set(&out->H_X, xrow, qcoord(a,     b + 1, n_cols));
                irrep_parity_matrix_set(&out->H_X, xrow, qcoord(a + 1, b + 1, n_cols));
                ++xrow;
            }
        }
    }
    for (int b = 0; b < n_cols - 1; ++b) {
        if ((b & 1) == 1) {
            irrep_parity_matrix_set(&out->H_X, xrow, qcoord(0, b,     n_cols));
            irrep_parity_matrix_set(&out->H_X, xrow, qcoord(0, b + 1, n_cols));
            ++xrow;
        }
    }
    for (int b = 0; b < n_cols - 1; ++b) {
        if (((n_rows - 1 + b) & 1) == 0) {
            irrep_parity_matrix_set(&out->H_X, xrow, qcoord(n_rows - 1, b,     n_cols));
            irrep_parity_matrix_set(&out->H_X, xrow, qcoord(n_rows - 1, b + 1, n_cols));
            ++xrow;
        }
    }

    int zrow = 0;
    for (int a = 0; a < n_rows - 1; ++a) {
        for (int b = 0; b < n_cols - 1; ++b) {
            if (((a + b) & 1) == 1) {
                irrep_parity_matrix_set(&out->H_Z, zrow, qcoord(a,     b,     n_cols));
                irrep_parity_matrix_set(&out->H_Z, zrow, qcoord(a + 1, b,     n_cols));
                irrep_parity_matrix_set(&out->H_Z, zrow, qcoord(a,     b + 1, n_cols));
                irrep_parity_matrix_set(&out->H_Z, zrow, qcoord(a + 1, b + 1, n_cols));
                ++zrow;
            }
        }
    }
    for (int a = 0; a < n_rows - 1; ++a) {
        if ((a & 1) == 0) {
            irrep_parity_matrix_set(&out->H_Z, zrow, qcoord(a,     0, n_cols));
            irrep_parity_matrix_set(&out->H_Z, zrow, qcoord(a + 1, 0, n_cols));
            ++zrow;
        }
    }
    for (int a = 0; a < n_rows - 1; ++a) {
        if (((a + n_cols - 1) & 1) == 1) {
            irrep_parity_matrix_set(&out->H_Z, zrow, qcoord(a,     n_cols - 1, n_cols));
            irrep_parity_matrix_set(&out->H_Z, zrow, qcoord(a + 1, n_cols - 1, n_cols));
            ++zrow;
        }
    }
    return IRREP_OK;
}

/* ====================================================================
 * Smooth merge: two d × d patches abutted along their right/left
 * smooth (Z-type) boundaries; merged geometry is d × 2d.
 * ==================================================================== */

irrep_status_t
irrep_lattice_surgery_smooth_init(irrep_lattice_surgery_smooth_t *out, int d)
{
    if (out == NULL || d < 2) return IRREP_ERR_INVALID_ARG;
    out->d = d;
    out->rows = d;
    out->cols = 2 * d;
    out->n_qubits = 2 * d * d;
    out->n_X_stabs = count_bulk_X(d, 2 * d)
                   + count_top_X(2 * d)
                   + count_bottom_X(d, 2 * d);
    out->n_Z_stabs = count_bulk_Z(d, 2 * d)
                   + count_left_Z(d)
                   + count_right_Z(d, 2 * d);
    return IRREP_OK;
}

irrep_status_t
irrep_lattice_surgery_smooth_build(const irrep_lattice_surgery_smooth_t *p,
                                   irrep_css_code_t *out)
{
    if (p == NULL || out == NULL || p->d < 2) return IRREP_ERR_INVALID_ARG;
    irrep_status_t s = irrep_css_code_new(out, p->n_qubits,
                                          p->n_X_stabs, p->n_Z_stabs);
    if (s != IRREP_OK) return s;
    return build_rect_surface_code(out, p->rows, p->cols);
}

irrep_status_t
irrep_lattice_surgery_smooth_logical_X(const irrep_lattice_surgery_smooth_t *p,
                                       irrep_pauli_t *out)
{
    if (p == NULL || out == NULL || p->d < 2) return IRREP_ERR_INVALID_ARG;
    irrep_status_t s = irrep_pauli_new(out, p->n_qubits);
    if (s != IRREP_OK) return s;
    /* L̄_X(M) = X-string on column 0 spanning all d rows (between the
     * top and bottom rough X-boundaries). Matches `irrep_surface_logical_X`. */
    for (int r = 0; r < p->rows; ++r) {
        irrep_pauli_set(out, qcoord(r, 0, p->cols), IRREP_PAULI_LETTER_X);
    }
    return IRREP_OK;
}

irrep_status_t
irrep_lattice_surgery_smooth_logical_Z(const irrep_lattice_surgery_smooth_t *p,
                                       irrep_pauli_t *out)
{
    if (p == NULL || out == NULL || p->d < 2) return IRREP_ERR_INVALID_ARG;
    irrep_status_t s = irrep_pauli_new(out, p->n_qubits);
    if (s != IRREP_OK) return s;
    /* L̄_Z(M) = Z-string on row 0 spanning all 2d columns (between the
     * left and right smooth Z-boundaries — now `2d` apart after merge).
     * This equals `Z̄_A · Z̄_B`: the smooth merge collapses both logical-Z
     * operators into the merged L̄_Z. */
    for (int c = 0; c < p->cols; ++c) {
        irrep_pauli_set(out, qcoord(0, c, p->cols), IRREP_PAULI_LETTER_Z);
    }
    return IRREP_OK;
}

irrep_status_t
irrep_lattice_surgery_smooth_joint_X_parity(
    const irrep_lattice_surgery_smooth_t *p, irrep_pauli_t *out)
{
    if (p == NULL || out == NULL || p->d < 2) return IRREP_ERR_INVALID_ARG;
    irrep_status_t s = irrep_pauli_new(out, p->n_qubits);
    if (s != IRREP_OK) return s;
    /* P_joint = X̄_A · X̄_B = X on column 0 AND column d of the merged frame.
     * Pre-merge: column 0 is A's logical-X column (X̄_A), column d is B's
     * logical-X column (X̄_B). After smooth merge their product becomes a
     * stabilizer — the joint-X parity measurement. */
    const int d = p->d;
    const int n_cols = p->cols;
    for (int r = 0; r < d; ++r) {
        irrep_pauli_set(out, qcoord(r, 0, n_cols), IRREP_PAULI_LETTER_X);
        irrep_pauli_set(out, qcoord(r, d, n_cols), IRREP_PAULI_LETTER_X);
    }
    return IRREP_OK;
}

/* ====================================================================
 * Rough merge: two d × d patches stacked vertically, A on top and B
 * on bottom; A's bottom rough (X-type) boundary meets B's top rough
 * boundary. Merged geometry is 2d × d.
 *
 * Measures the joint-Z parity Z̄_A · Z̄_B = Z on row 0 AND row d of the
 * merged frame, which becomes a stabilizer of the merged code.
 * ==================================================================== */

irrep_status_t
irrep_lattice_surgery_rough_init(irrep_lattice_surgery_rough_t *out, int d)
{
    if (out == NULL || d < 2) return IRREP_ERR_INVALID_ARG;
    out->d = d;
    out->rows = 2 * d;
    out->cols = d;
    out->n_qubits = 2 * d * d;
    out->n_X_stabs = count_bulk_X(2 * d, d)
                   + count_top_X(d)
                   + count_bottom_X(2 * d, d);
    out->n_Z_stabs = count_bulk_Z(2 * d, d)
                   + count_left_Z(2 * d)
                   + count_right_Z(2 * d, d);
    return IRREP_OK;
}

irrep_status_t
irrep_lattice_surgery_rough_build(const irrep_lattice_surgery_rough_t *p,
                                  irrep_css_code_t *out)
{
    if (p == NULL || out == NULL || p->d < 2) return IRREP_ERR_INVALID_ARG;
    irrep_status_t s = irrep_css_code_new(out, p->n_qubits,
                                          p->n_X_stabs, p->n_Z_stabs);
    if (s != IRREP_OK) return s;
    return build_rect_surface_code(out, p->rows, p->cols);
}

irrep_status_t
irrep_lattice_surgery_rough_logical_X(const irrep_lattice_surgery_rough_t *p,
                                      irrep_pauli_t *out)
{
    if (p == NULL || out == NULL || p->d < 2) return IRREP_ERR_INVALID_ARG;
    irrep_status_t s = irrep_pauli_new(out, p->n_qubits);
    if (s != IRREP_OK) return s;
    /* L̄_X(M) = X-string on column 0 spanning all 2d rows (between the
     * top and bottom rough X-boundaries — now 2d apart after merge).
     * This equals X̄_A · X̄_B: the rough merge collapses both logical-X
     * operators into the merged L̄_X. */
    for (int r = 0; r < p->rows; ++r) {
        irrep_pauli_set(out, qcoord(r, 0, p->cols), IRREP_PAULI_LETTER_X);
    }
    return IRREP_OK;
}

irrep_status_t
irrep_lattice_surgery_rough_logical_Z(const irrep_lattice_surgery_rough_t *p,
                                      irrep_pauli_t *out)
{
    if (p == NULL || out == NULL || p->d < 2) return IRREP_ERR_INVALID_ARG;
    irrep_status_t s = irrep_pauli_new(out, p->n_qubits);
    if (s != IRREP_OK) return s;
    /* L̄_Z(M) = Z-string on row 0 spanning all d columns (matches
     * `irrep_surface_logical_Z` — single d-row, between the left and
     * right smooth Z-boundaries). */
    for (int c = 0; c < p->cols; ++c) {
        irrep_pauli_set(out, qcoord(0, c, p->cols), IRREP_PAULI_LETTER_Z);
    }
    return IRREP_OK;
}

irrep_status_t
irrep_lattice_surgery_rough_joint_Z_parity(
    const irrep_lattice_surgery_rough_t *p, irrep_pauli_t *out)
{
    if (p == NULL || out == NULL || p->d < 2) return IRREP_ERR_INVALID_ARG;
    irrep_status_t s = irrep_pauli_new(out, p->n_qubits);
    if (s != IRREP_OK) return s;
    /* P_joint = Z̄_A · Z̄_B = Z on row 0 AND row d of the merged frame
     * (each row covering c ∈ [0, d)). Pre-merge: row 0 is A's logical-Z
     * row (Z̄_A), row d is B's logical-Z row (Z̄_B). After rough merge
     * their product becomes a stabilizer — the joint-Z parity measurement. */
    const int d = p->d;
    const int n_cols = p->cols;
    for (int c = 0; c < d; ++c) {
        irrep_pauli_set(out, qcoord(0, c, n_cols), IRREP_PAULI_LETTER_Z);
        irrep_pauli_set(out, qcoord(d, c, n_cols), IRREP_PAULI_LETTER_Z);
    }
    return IRREP_OK;
}
