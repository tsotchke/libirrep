...
irrep_multiset_2j_t *irrep_multiset_2j_new(int capacity) {
    ...
    if (capacity > 0) {
        ...
        if (!m->labels || !m->multiplicities) {
            free(m->labels); free(m->multiplicities); free(m);
            return NULL;
        }
    }
    m->capacity = capacity;
...
static int grow_(irrep_multiset_2j_t *m, int needed) {
    if (needed <= m->capacity) return 0;
    int new_cap = m->capacity ? m->capacity * 2 : 4;
    while (new_cap < needed) new_cap *= 2;
    irrep_label_2j_t *nl = realloc(m->labels,
                                    (size_t)new_cap * sizeof(*m->labels));
    int *nm = realloc(m->multiplicities,
                      (size_t)new_cap * sizeof(*m->multiplicities));
    if (!nl || !nm) { free(nl); free(nm); return -1; }
    m->labels = nl; m->multiplicities = nm; m->capacity = new_cap;
    return 0;
}

irrep_status_t irrep_multiset_2j_append(irrep_multiset_2j_t *m,
                                        irrep_label_2j_t label,
                                        int multiplicity) {
    if (!m)                          return IRREP_ERR_INVALID_ARG;
    if (label.two_j < 0)             return IRREP_ERR_INVALID_ARG;
    if (label.parity != +1 && label.parity != -1) return IRREP_ERR_INVALID_ARG;
    ...
    if (multiplicity > INT_MAX / block_dim) return IRREP_ERR_INVALID_ARG;
    long added = (long)multiplicity * block_dim;
    if (m->total_dim > INT_MAX - added)     return IRREP_ERR_INVALID_ARG;

    if (grow_(m, m->num_terms + 1) != 0) return IRREP_ERR_OUT_OF_MEMORY;
...
irrep_multiset_2j_t *irrep_multiset_2j_parse(const char *spec) {
    ...
    const char *p = spec;
    skip_ws(&p);
    if (*p == '\0') return m;       /* empty spec → empty multiset */
    ...
    for (;;) {
        ...
        if (parse_uint_(&p, &mult) != 0 || mult <= 0 || mult > INT_MAX) {
            irrep_set_error_("irrep_multiset_2j_parse: expected positive "
                             ...
                             (long)(p - spec + 1));
            irrep_multiset_2j_free(m); return NULL;
        }
        if (*p != 'x') {
...
int irrep_multiset_2j_format(const irrep_multiset_2j_t *m, char *buf,
                              size_t buflen) {
    ...
    for (int i = 0; i < m->num_terms; ++i) {
        ...
        int n;
        if (lbl->two_j & 1) {
            n = snprintf(label_buf, sizeof(label_buf), "%dx%d/2%c",
                         m->multiplicities[i], lbl->two_j,
                         lbl->parity > 0 ? 'e' : 'o');
        } else {
            n = snprintf(label_buf, sizeof(label_buf), "%dx%d%c",
                         m->multiplicities[i], lbl->two_j / 2,
                         lbl->parity > 0 ? 'e' : 'o');
        }
        if (n < 0) return (int)written;
        int sep = (i > 0);
        if (sep) {
            if (written + 3 < buflen) { buf[written++] = ' '; buf[written++] = '+'; buf[written++] = ' '; }
            else written += 3;
        }
    ...
    if (written < buflen) buf[written] = '\0'; else buf[buflen - 1] = '\0';
    return (int)written;
}
...
int irrep_multiset_2j_has_half_integer(const irrep_multiset_2j_t *m) {
    if (!m) return 0;
    for (int i = 0; i < m->num_terms; ++i) {
        if (m->labels[i].two_j & 1) return 1;
    }
    return 0;
}
