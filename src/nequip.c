...
irrep_nequip_layer_t *irrep_nequip_layer_build(
    ...
    if (!hidden_in || !hidden_out) {
        irrep_set_error_("irrep_nequip_layer_build: NULL multiset input"); return NULL;
    }
    if (l_sh_max < 0 || r_cut <= 0.0 || n_radial <= 0) {
        irrep_set_error_("irrep_nequip_layer_build: invalid numeric arg"); return NULL;
    }
    ...
    if (!layer->sh_mset || !layer->hidden_in_copy || !layer->hidden_out_copy) {
        irrep_nequip_layer_free(layer); return NULL;
    }
    ...
    int n_paths = irrep_tp_enumerate_paths(layer->hidden_in_copy,
                                            layer->sh_mset,
                                            layer->hidden_out_copy, NULL, 0);
    int *paths = malloc((size_t)(n_paths > 0 ? n_paths : 1) * 3 * sizeof(int));
    if (!paths) { irrep_nequip_layer_free(layer); return NULL; }
    irrep_tp_enumerate_paths(layer->hidden_in_copy, layer->sh_mset,
                              layer->hidden_out_copy, paths, n_paths);
    layer->tp = irrep_tp_build_uvw(layer->hidden_in_copy, layer->sh_mset,
                                    layer->hidden_out_copy, paths, n_paths);
    free(paths);
    if (!layer->tp) { irrep_nequip_layer_free(layer); return NULL; }
    return layer;
}
...
void irrep_nequip_layer_apply(
    ...
    if (!sh_buf || !msg) { free(sh_buf); free(msg); return; }
    ...
    for (int e = 0; e < n_edges; ++e) {
        ...
        if (r <= 0.0 || r >= layer->r_cut) continue;
        double inv_r = 1.0 / r;
        ...
        irrep_tp_apply_uvw(layer->tp, tp_weights,
                           h_in + (size_t)src * layer->h_in_dim,
                           sh_buf,
                           msg);
        /* accumulate into h_out[dst]: h_out[dst] += scale · msg */
        double *target = h_out + (size_t)dst * layer->h_out_dim;
        for (int k = 0; k < layer->h_out_dim; ++k) target[k] += scale * msg[k];
    }
...
void irrep_nequip_layer_apply_backward(
    const irrep_nequip_layer_t *layer,
    const double               *tp_weights,
    int                         n_nodes,
    int                         n_edges,
    ...
    if (!sh_buf || !scaled_g_out || !ga_local) {
        free(sh_buf); free(scaled_g_out); free(ga_local);
        return;
    }
    ...
    for (int e = 0; e < n_edges; ++e) {
        ...
        irrep_tp_apply_uvw_backward(layer->tp, tp_weights,
                                     h_in + (size_t)src * layer->h_in_dim,
                                     sh_buf,
                                     scaled_g_out,
                                     grad_h_in ? ga_local : NULL,
                                     NULL,                     /* skip grad of SH */
                                     grad_tp_weights);
        if (grad_h_in) {
            double *gh = grad_h_in + (size_t)src * layer->h_in_dim;
            for (int k = 0; k < layer->h_in_dim; ++k) gh[k] += ga_local[k];
        }
    }
...
void irrep_nequip_layer_apply_forces(
    const irrep_nequip_layer_t *layer,
    const double               *tp_weights,
    int                         n_nodes,
    int                         n_edges,
    ...
    if (!sh_buf || !grad_sh || !msg || !scaled_g_out || !sh_grad) {
        free(sh_buf); free(grad_sh); free(msg);
        free(scaled_g_out); free(sh_grad);
        return;
    }
    ...
    for (int e = 0; e < n_edges; ++e) {
        ...
        double dot_gm = 0.0;
        for (int k = 0; k < h_out_dim; ++k) dot_gm += go_dst[k] * msg[k];
        double coeff_r = dot_gm * scale_d;

        /* Second term: SH-side pullback. scaled_g_out = scale · grad_h_out[dst]. */
        for (int k = 0; k < h_out_dim; ++k) scaled_g_out[k] = scale * go_dst[k];
        ...
        for (int axis = 0; axis < 3; ++axis) {
            ...
            for (int m = 0; m < sh_dim; ++m) s += grad_sh[m] * gb_axis[m];
