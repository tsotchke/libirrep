...
static long consume_int_(cursor_t *c) {
    if (!isdigit((unsigned char)*c->p)) return -1;
    char *end = NULL;
...
static const char *find_arrow_outside_brackets_(const char *s) {
    ...
    for (const char *q = s; *q; ++q) {
        ...
        else if (depth == 0 && q[0] == '-' && q[1] == '>') return q;
    }
    return NULL;
}

/* Parse the [option, option, ...] block. On entry `c->p` is at `[`. */
static int parse_options_(cursor_t *c,
                          int                    *out_sh,
                          int                    *out_radial,
                          double                 *out_r_cut,
                          irrep_nequip_cutoff_t  *out_cutoff_kind,
                          int                    *out_cutoff_p) {
    if (expect_char_(c, '[') != 0) return -1;
    ...
    for (;;) {
        ...
        if (nk < 0) {
            irrep_set_error_("nequip spec: expected option name at column %d",
                             col_of_(c));
            return -1;
        }
        if (expect_char_(c, '=') != 0) {
            irrep_set_error_("nequip spec: expected '=' after option name "
                             "'%s' at column %d", name, col_of_(c));
            return -1;
        }
        skip_ws_(c);
        ...
        if (strcmp(name, "sh") == 0) {
            ...
            if (v < 0 || v > INT_MAX) {
                irrep_set_error_("nequip spec: 'sh' expects non-negative "
                                 "integer ≤ INT_MAX at column %d", col_of_(c));
                return -1;
            }
            *out_sh = (int)v;
            ...
            *out_r_cut = v;
                ...
                *out_cutoff_kind = IRREP_NEQUIP_CUTOFF_COSINE;
                ...
                if (v < 1 || v > INT_MAX) {
                    irrep_set_error_("nequip spec: polynomial order must be "
                                     "in [1, INT_MAX] at column %d", col_of_(c));
                    return -1;
                }
                if (expect_char_(c, ')') != 0) {
            ...
            irrep_set_error_("nequip spec: unknown option '%s' at column %d",
                             name, col_of_(c));
            return -1;
        }
        ...
        if (*c->p == ',') { ++c->p; continue; }
        if (*c->p == ']') { ++c->p; break; }
        irrep_set_error_("nequip spec: expected ',' or ']' at column %d",
                         col_of_(c));
        return -1;
    }
    ...
    skip_ws_(c);
    if (*c->p != '\0') {
        irrep_set_error_("nequip spec: unexpected trailing input at column %d",
                         col_of_(c));
        return -1;
    }
    return 0;
...
irrep_nequip_layer_t *irrep_nequip_layer_from_spec(const char *spec) {
    ...
    if (!h_in || !h_out) {
        ...
        irrep_set_error_("nequip spec: failed to parse multiset — %s",
                         irrep_last_error());
        return NULL;
    }
    ...
    if (*rhs_end == '[') {
        ...
        if (parse_options_(&c, &sh_max, &n_radial, &r_cut,
                           &cutoff_kind, &cutoff_poly) != 0) {
            irrep_multiset_free(h_in);
            irrep_multiset_free(h_out);
            return NULL;
        }
    ...
    irrep_nequip_layer_t *layer = irrep_nequip_layer_build(
        h_in, sh_max, n_radial, r_cut, cutoff_kind, cutoff_poly, h_out);
