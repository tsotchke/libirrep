...
static int cache_block_size_(int l_max) {
    int s = 0;
    for (int l = 0; l <= l_max; ++l) s += (2 * l + 1) * (2 * l + 1);
    return s;
}

static int cache_block_offset_(int l) {
    int s = 0;
    for (int ll = 0; ll < l; ++ll) s += (2 * ll + 1) * (2 * ll + 1);
    return s;
}
...
static void fill_c4v_elements_(pg_element_t *e) {
    e[0].R = irrep_rot_identity();              e[0].det = +1;   /* E   */
    e[1].R = rot_z_( M_PI / 2.0);               e[1].det = +1;   /* C4  */
    e[2].R = rot_z_( M_PI);                     e[2].det = +1;   /* C2  */
    e[3].R = rot_z_(-M_PI / 2.0);               e[3].det = +1;   /* C4³ */
    ...
    e[4].R = rot_axis_(0.0, 1.0, 0.0, M_PI);    e[4].det = -1;   /* σ_v(xz) */
    e[5].R = rot_axis_(1.0, 0.0, 0.0, M_PI);    e[5].det = -1;   /* σ_v(yz) */
    /* σ_d(x=y): maps (x,y,z) → (y,x,-z). That equals inversion · Rn(π) where
     * n is the axis along (1, -1, 0)/√2. */
    e[6].R = rot_axis_(1.0, -1.0, 0.0, M_PI);   e[6].det = -1;   /* σ_d(x=y) */
    e[7].R = rot_axis_(1.0,  1.0, 0.0, M_PI);   e[7].det = -1;   /* σ_d(x=-y) */
}
...
static void fill_c3v_elements_(pg_element_t *e) {
    e[0].R = irrep_rot_identity();                 e[0].det = +1;   /* E   */
    e[1].R = rot_z_( 2.0 * M_PI / 3.0);            e[1].det = +1;   /* C3  */
    e[2].R = rot_z_(-2.0 * M_PI / 3.0);            e[2].det = +1;   /* C3² */
...
irrep_pg_table_t *irrep_pg_table_build(irrep_point_group_t g) {
    ...
    if (!t->characters) { free(t->elements); free(t); return NULL; }
    ...
    if (t->D_cache) {
        for (int e = 0; e < t->order; ++e) {
            for (int l = 0; l <= IRREP_PG_CACHED_L_MAX; ++l) {
                double *slot = t->D_cache
                             + (size_t)e * (size_t)t->D_cache_stride
                             + cache_block_offset_(l);
...
int irrep_pg_num_irreps(const irrep_pg_table_t *t) { return t ? t->num_irreps : 0; }
int irrep_pg_order     (const irrep_pg_table_t *t) { return t ? t->order      : 0; }
const char *irrep_pg_irrep_label(const irrep_pg_table_t *t, int mu) {
    if (!t || mu < 0 || mu >= t->num_irreps) return NULL;
...
void irrep_pg_project(const irrep_pg_table_t *t, int mu,
                      const irrep_multiset_t *spec,
                      const double *in, double *out) {
    if (!t || !spec || !in || !out) return;
    if (mu < 0 || mu >= t->num_irreps) return;
    ...
    for (int e_idx = 0; e_idx < t->order; ++e_idx) {
        ...
        for (int ti = 0; ti < spec->num_terms; ++ti) {
            ...
            if (l <= IRREP_PG_CACHED_L_MAX && t->D_cache) {
                D_block = t->D_cache
                        + (size_t)e_idx * (size_t)t->D_cache_stride
                        + cache_block_offset_(l);
            } else {
            ...
            double coeff = w * flip;
            if (coeff == 0.0) { offset += mult * d; continue; }
            ...
            for (int u = 0; u < mult; ++u) {
                ...
                for (int i = 0; i < d; ++i) {
                    double s = 0.0;
                    for (int j = 0; j < d; ++j) s += D_block[i * d + j] * src[j];
                    block_tmp[i] = s;
                }
                for (int i = 0; i < d; ++i) dst[i] += coeff * block_tmp[i];
            }
            offset += mult * d;
        }
    }
...
static double chi_l_(int l, double theta) {
    if (fabs(theta) < 1e-14) return (double)(2 * l + 1);
...
void irrep_pg_reduce(const irrep_pg_table_t *t,
                     const irrep_multiset_t *spec, int *out_mult) {
    if (!t || !spec || !out_mult) return;

    /* Precompute χ_V(g) over all group elements. */
    double *chi_V = calloc((size_t)t->order, sizeof(double));
    if (!chi_V) { for (int m = 0; m < t->num_irreps; ++m) out_mult[m] = 0; return; }
    for (int e_idx = 0; e_idx < t->order; ++e_idx) {
        const pg_element_t *g = &t->elements[e_idx];
        ...
        for (int ti = 0; ti < spec->num_terms; ++ti) {
            ...
            if (g->det < 0) chi_block *= (double)par;
    ...
    for (int mu = 0; mu < t->num_irreps; ++mu) {
        const double *chi = t->characters + (size_t)mu * (size_t)t->order;
        double acc = 0.0;
        for (int e_idx = 0; e_idx < t->order; ++e_idx) acc += chi[e_idx] * chi_V[e_idx];
