...
void irrep_rbf_bessel_d_batch(int n, size_t N, const double *r,
                               double r_cut, double *out) {
    for (size_t i = 0; i < N; ++i) out[i] = irrep_rbf_bessel_d(n, r[i], r_cut);
}
...
void irrep_rbf_bessel_batch_scalar(int n, size_t N, const double *r,
                                   double r_cut, double *out) {
    for (size_t i = 0; i < N; ++i) out[i] = irrep_rbf_bessel(n, r[i], r_cut);
}

void irrep_cutoff_cosine_batch_scalar(size_t N, const double *r,
                                      double r_cut, double *out) {
    for (size_t i = 0; i < N; ++i) out[i] = irrep_cutoff_cosine(r[i], r_cut);
}

void irrep_cutoff_polynomial_batch_scalar(size_t N, const double *r,
                                          double r_cut, int p, double *out) {
    for (size_t i = 0; i < N; ++i) out[i] = irrep_cutoff_polynomial(r[i], r_cut, p);
}

void irrep_cutoff_cosine_d_batch_scalar(size_t N, const double *r,
                                        double r_cut, double *out) {
    for (size_t i = 0; i < N; ++i) out[i] = irrep_cutoff_cosine_d(r[i], r_cut);
}

void irrep_cutoff_polynomial_d_batch_scalar(size_t N, const double *r,
                                            double r_cut, int p, double *out) {
    for (size_t i = 0; i < N; ++i) out[i] = irrep_cutoff_polynomial_d(r[i], r_cut, p);
}

void irrep_rbf_bessel_batch(int n, size_t N, const double *r,
                             double r_cut, double *out) {
    irrep_dispatch_get()->rbf_bessel_batch(n, N, r, r_cut, out);
}

void irrep_cutoff_cosine_batch(size_t N, const double *r,
                                double r_cut, double *out) {
    irrep_dispatch_get()->cutoff_cosine_batch(N, r, r_cut, out);
}

void irrep_cutoff_polynomial_batch(size_t N, const double *r,
                                    double r_cut, int p, double *out) {
    irrep_dispatch_get()->cutoff_polynomial_batch(N, r, r_cut, p, out);
}

void irrep_cutoff_cosine_d_batch(size_t N, const double *r,
                                  double r_cut, double *out) {
    irrep_dispatch_get()->cutoff_cosine_d_batch(N, r, r_cut, out);
}

void irrep_cutoff_polynomial_d_batch(size_t N, const double *r,
                                      double r_cut, int p, double *out) {
