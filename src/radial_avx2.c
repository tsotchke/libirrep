 ...
 */
void irrep_cutoff_polynomial_batch_avx2(size_t N, const double *r,
    ...
    for (; i + 4 <= N; i += 4) {
        ...
        __m256d vup = vu;
        for (int k = 1; k < p; ++k) vup = _mm256_mul_pd(vup, vu);
...
void irrep_cutoff_polynomial_d_batch_avx2(size_t N, const double *r,
                                           double r_cut, int p, double *out) {
    if (r_cut <= 0.0 || p < 1) {
        for (size_t i = 0; i < N; ++i) out[i] = 0.0;
        return;
    ...
    for (; i + 4 <= N; i += 4) {
        __m256d vr = _mm256_loadu_pd(r + i);
        __m256d valid = _mm256_and_pd(
            _mm256_cmp_pd(vr, vzero,  _CMP_GE_OS),
            _mm256_cmp_pd(vr, vr_cut, _CMP_LT_OS));

        __m256d vu = _mm256_mul_pd(vr, vinv_rcut);

        __m256d vupm1 = vone;
        for (int k = 0; k < p - 1; ++k) vupm1 = _mm256_mul_pd(vupm1, vu);
