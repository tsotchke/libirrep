...
void irrep_cutoff_polynomial_batch_neon(size_t N, const double *r, double r_cut, int p,
                                        double *out) {
    if (r_cut <= 0.0 || p < 1) {
    ...
    for (; i + 2 <= N; i += 2) {
        ...
        uint64x2_t  valid = vandq_u64(vcgeq_f64(vr, vzero), vcltq_f64(vr, vdupq_n_f64(r_cut)));
        ...
        t = vreinterpretq_f64_u64(vandq_u64(vreinterpretq_u64_f64(t), valid));
...
void irrep_cutoff_polynomial_d_batch_neon(size_t N, const double *r, double r_cut, int p,
                                          double *out) {
    if (r_cut <= 0.0 || p < 1) {
        for (size_t i = 0; i < N; ++i)
            out[i] = 0.0;
