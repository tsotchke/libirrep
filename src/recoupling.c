...
static int triangle_2j_(int two_j1, int two_j2, int two_j3) {
    if (two_j1 < 0 || two_j2 < 0 || two_j3 < 0) return 0;
    if ((two_j1 + two_j2 + two_j3) & 1) return 0;
    if (two_j3 < iabs_(two_j1 - two_j2)) return 0;
    if (two_j3 > two_j1 + two_j2) return 0;
    return 1;
}
...
double irrep_wigner_6j_2j(int two_j1, int two_j2, int two_j3,
                          int two_j4, int two_j5, int two_j6) {
    if (!triangle_2j_(two_j1, two_j2, two_j3)) return 0.0;
    if (!triangle_2j_(two_j4, two_j5, two_j3)) return 0.0;
    if (!triangle_2j_(two_j4, two_j2, two_j6)) return 0.0;
    if (!triangle_2j_(two_j1, two_j5, two_j6)) return 0.0;
    ...
    double log_deltas = 0.5 * (log_delta_sq_2j_(two_j1, two_j2, two_j3)
                             + log_delta_sq_2j_(two_j4, two_j5, two_j3)
                             + log_delta_sq_2j_(two_j4, two_j2, two_j6)
                             + log_delta_sq_2j_(two_j1, two_j5, two_j6));

    double sum = 0.0;
    for (int t = t_min; t <= t_max; ++t) {
        double log_term = lgamma(t + 2)
                        - lgamma(t - a + 1) - lgamma(t - b + 1)
                        - lgamma(t - c + 1) - lgamma(t - d + 1)
                        - lgamma(e - t + 1) - lgamma(f - t + 1)
                        - lgamma(g - t + 1);
 ...
 */
double irrep_wigner_9j_2j(int two_j1, int two_j2, int two_j3,
                          int two_j4, int two_j5, int two_j6,
                          int two_j7, int two_j8, int two_j9) {
    int two_k_max = two_j1 + two_j9;
    if (two_j4 + two_j8 < two_k_max) two_k_max = two_j4 + two_j8;
    if (two_j2 + two_j6 < two_k_max) two_k_max = two_j2 + two_j6;

    int two_k_min = 0;
    int a = iabs_(two_j1 - two_j9);
    int b = iabs_(two_j4 - two_j8);
    int c = iabs_(two_j2 - two_j6);
    if (a > two_k_min) two_k_min = a;
    if (b > two_k_min) two_k_min = b;
    if (c > two_k_min) two_k_min = c;
    if (two_k_min > two_k_max) return 0.0;

    double sum = 0.0;
    for (int two_k = two_k_min; two_k <= two_k_max; ++two_k) {
        double s1 = irrep_wigner_6j_2j(two_j1, two_j4, two_j7,
                                       two_j8, two_j9, two_k);
        if (s1 == 0.0) continue;
        double s2 = irrep_wigner_6j_2j(two_j2, two_j5, two_j8,
                                       two_j4, two_k, two_j6);
        if (s2 == 0.0) continue;
        double s3 = irrep_wigner_6j_2j(two_j3, two_j6, two_j9,
                                       two_k, two_j1, two_j2);
        if (s3 == 0.0) continue;
...
double irrep_wigner_9j(int j1, int j2, int j3,
                       int j4, int j5, int j6,
                       int j7, int j8, int j9) {
    return irrep_wigner_9j_2j(2*j1, 2*j2, 2*j3,
                              2*j4, 2*j5, 2*j6,
                              2*j7, 2*j8, 2*j9);
}
