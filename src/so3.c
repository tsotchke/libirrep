...
irrep_quaternion_t irrep_quat_frechet_mean(const irrep_quaternion_t *qs, const double *weights,
                                           size_t n) {
...
bool irrep_rot_is_valid(irrep_rot_matrix_t R, double tolerance) {
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            double s = 0.0;
            for (int k = 0; k < 3; ++k)
                s += R.m[k * 3 + i] * R.m[k * 3 + j];
            double target = (i == j) ? 1.0 : 0.0;
            if (fabs(s - target) > tolerance)
                return false;
        }
    }
    /* det(R) = +1 */
