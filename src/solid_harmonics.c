...
static void solid_recurrence_fill_(int l_max, const double r[3],
                                   int want_grad,
    ...
    for (int l = 0; l <= l_max; ++l) {
        for (int j = 0; j < stride; ++j) R_all[l * stride + j] = 0.0;
    }
    if (want_grad && dR_all) {
    ...
    for (int l = 2; l <= l_max; ++l) {
        ...
        for (int m = 0; m <= l - 2; ++m) {
            ...
            if (l >= 2) {
                ...
                if (denD > 0.0 && numD >= 0.0) D = sqrt(numD / denD);
            }
...
void irrep_solid_harm_cart(int l, double *out, const double r[3]) {
    init_constants_();
    if (l < 0 || l > IRREP_SOLID_L_MAX || !out) return;
    double x = r[0], y = r[1], z = r[2];

    switch (l) {
        case 0: solid_harm_l0_(out);           return;
        case 1: solid_harm_l1_(out, x, y, z);  return;
        case 2: solid_harm_l2_(out, x, y, z);  return;
        case 3: solid_harm_l3_(out, x, y, z);  return;
        case 4: solid_harm_l4_(out, x, y, z);  return;
        default: break;
    }
...
void irrep_solid_harm_cart_all(int l_max, double *out, const double r[3]) {
    if (l_max < 0 || l_max > IRREP_SOLID_L_MAX || !out) return;
    int offset = 0;
    for (int l = 0; l <= l_max; ++l) {
...
static void solid_grad_l0_(double *out) {
    for (int axis = 0; axis < 3; ++axis) out[axis] = 0.0;
}
...
static void solid_grad_l2_(double *out, double x, double y, double z) {
    double g[3][5];
    /* m = -2: R = K2m2 · xy */
    g[0][0] = K2m2 * y;  g[1][0] = K2m2 * x;  g[2][0] = 0.0;
    /* m = -1: R = K2m2 · yz */
    g[0][1] = 0.0;       g[1][1] = K2m2 * z;  g[2][1] = K2m2 * y;
    /* m =  0: R = K20 · (2z² - x² - y²) */
    g[0][2] = K20 * (-2.0 * x);  g[1][2] = K20 * (-2.0 * y);  g[2][2] = K20 * 4.0 * z;
    /* m = +1: R = K2m2 · xz */
    g[0][3] = K2m2 * z;  g[1][3] = 0.0;       g[2][3] = K2m2 * x;
    /* m = +2: R = K22 · (x² - y²) */
    g[0][4] = K22 * 2.0 * x;  g[1][4] = K22 * (-2.0 * y);  g[2][4] = 0.0;
...
static void solid_grad_l3_(double *out, double x, double y, double z) {
    ...
    g[2][0] = 0.0;
    /* m = -2: R = K3m2 · xyz */
    g[0][1] = K3m2 * y * z;  g[1][1] = K3m2 * x * z;  g[2][1] = K3m2 * x * y;
    /* m = -1: R = K3m1 · y · (4z² − x² − y²) */
    g[0][2] = K3m1 * (-2.0 * x * y);
...
void irrep_solid_harm_cart_grad(int l, double *out, const double r[3]) {
    ...
    switch (l) {
        case 0: solid_grad_l0_(out);                    return;
        case 1: solid_grad_l1_(out);                    return;
        case 2: solid_grad_l2_(out, r[0], r[1], r[2]);  return;
        case 3: solid_grad_l3_(out, r[0], r[1], r[2]);  return;
        case 4: solid_grad_l4_(out, r[0], r[1], r[2]);  return;
        default: break;
    }
