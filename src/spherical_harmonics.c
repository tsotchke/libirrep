...
double irrep_legendre_assoc(int l, int m, double x) {
    ...
    if (am > 0) {
        double fact = 1.0;
        for (int k = 1; k <= 2 * am - 1; k += 2) fact *= (double)k;
        double pow_sx = 1.0;
        for (int k = 0; k < am; ++k) pow_sx *= somx2;
    ...
    if (l == am) {
        ...
        if (l == am + 1) {
            ...
            for (int ll = am + 2; ll <= l; ++ll) {
                double val = ((2.0 * ll - 1.0) * x * p_lm1
                              - (double)(ll + am - 1) * p_lm2) / (double)(ll - am);
                p_lm2 = p_lm1;
                p_lm1 = val;
            }
            plm = p_lm1;
...
double irrep_sph_harm_real(int l, int m, double theta, double phi) {
    ...
    if (m > 0) return sign * sqrt2 * N * p * cos((double)am * phi);
...
void irrep_sph_harm_cart_f32(int l, float *out, const float r_hat[3]) {
    ...
    for (int i = 0; i < 2 * l + 1; ++i) out[i] = (float)tmp[i];
}
