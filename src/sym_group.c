...
int irrep_permutation_sign(const int *perm, int n) {
    if (!perm || n < 1) return 0;
    /* Validate: each of [0, n) exactly once. */
    int seen[32] = {0};
    if (n > 32) return 0;
    for (int i = 0; i < n; ++i) {
        if (perm[i] < 0 || perm[i] >= n || seen[perm[i]]) return 0;
        seen[perm[i]] = 1;
    }
...
irrep_status_t irrep_permutations_all(int n, int *out_perms) {
    if (n < 1 || n > 10 || !out_perms) return IRREP_ERR_INVALID_ARG;
    ...
    if (r != fact) return IRREP_ERR_PRECONDITION;
    return IRREP_OK;
}
...
long long irrep_young_dim(const int *partition, int n_parts) {
    if (!partition || n_parts < 1 || n_parts > 32) return -1;
    int N = 0;
    for (int i = 0; i < n_parts; ++i) {
        if (partition[i] <= 0) return -1;
        if (i > 0 && partition[i] > partition[i - 1]) return -1;
        N += partition[i];
    }
    if (N > 20) return -1;   /* factorial overflow */
    ...
    for (int i = 0; i < n_parts; ++i) {
        ...
        for (int j = 0; j < row_len; ++j) {
            ...
            for (int ii = i + 1; ii < n_parts; ++ii) {
                if (partition[ii] > j) ++leg; else break;
            }
            long long h = arm + leg + 1;
            hook_prod *= h;
            if (hook_prod <= 0) return -1;   /* overflow */
        }
    }

    long long Nfact = irrep_factorial(N);
    if (Nfact < 0 || hook_prod == 0) return -1;
...
static long long permute_index_(long long i, int N, int d,
                                const int *sigma, const long long *weight) {
    int digits[16];
    for (int k = 0; k < N; ++k) { digits[k] = (int)(i % d); i /= d; }
    long long out = 0;
    for (int k = 0; k < N; ++k) {
...
static irrep_status_t apply_projector_(int N, int d,
                                       int use_sign,
                                       const double _Complex *psi_in,
                                       double _Complex *psi_out) {
    if (N < 1 || N > 10 || d < 2 || !psi_in || !psi_out) return IRREP_ERR_INVALID_ARG;

    long long fact = irrep_factorial(N);
    long long dim  = ipow_ll_(d, N);

    int *perms = malloc((size_t)fact * (size_t)N * sizeof(int));
    if (!perms) return IRREP_ERR_OUT_OF_MEMORY;
    ...
    long long weight[16];
    for (int k = 0; k < N; ++k) weight[k] = ipow_ll_(d, k);
    ...
    if (psi_in == psi_out) {
        ...
        if (!psi_in_copy) { free(perms); return IRREP_ERR_OUT_OF_MEMORY; }
