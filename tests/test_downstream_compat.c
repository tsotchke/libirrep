...
static const char *load_config_(const char *p,
                                double m_i[3], double m_j[3], double r_hat[3],
                                double T1[3], double T2[3], double T3[3],
                                double T4[3], double T5[3]) {
    const char *q = p;
    q = find_(q, "\"m_i\":");        if (!q) return NULL;
    q += strlen("\"m_i\":");          if (parse_vec3_(&q, m_i)  != 0) return NULL;
    q = find_(q, "\"m_j\":");        if (!q) return NULL;
    q += strlen("\"m_j\":");          if (parse_vec3_(&q, m_j)  != 0) return NULL;
    q = find_(q, "\"r_hat\":");      if (!q) return NULL;
    q += strlen("\"r_hat\":");        if (parse_vec3_(&q, r_hat) != 0) return NULL;
    ...
    q = find_(q, "\"T1_dot_proj\":");    if (!q) return NULL; ++q;
    if (parse_vec3_(&q, T1) != 0) return NULL;
    q = find_(q, "\"T2_mj_cross_r\":");  if (!q) return NULL; ++q;
    if (parse_vec3_(&q, T2) != 0) return NULL;
    q = find_(q, "\"T3_mi_cross_mj\":"); if (!q) return NULL; ++q;
    if (parse_vec3_(&q, T3) != 0) return NULL;
    q = find_(q, "\"T4_mij_mi\":");      if (!q) return NULL; ++q;
    if (parse_vec3_(&q, T4) != 0) return NULL;
    q = find_(q, "\"T5_mj\":");          if (!q) return NULL; ++q;
    if (parse_vec3_(&q, T5) != 0) return NULL;
    return q;
}
...
static tp_descriptor_t *build_tp_(void) {
    ...
    free(paths);
    irrep_multiset_free(a); irrep_multiset_free(b); irrep_multiset_free(c);
    return tp;
}
...
static void tp_cross_(tp_descriptor_t *tp, const double *w,
                      const double cart_a[3], const double cart_b[3],
                      double cart_out[3]) {
    double a_real[3], b_real[3];
    cart_to_real_(cart_a, a_real);
...
int main(void) {
    ...
    for (;;) {
        ...
        if (load_config_(block, m_i, m_j, r_hat,
                         T1_ref, T2_ref, T3_ref, T4_ref, T5_ref) == NULL) break;

        /* T1 = (m_j · r̂) · m_i  via (1o⊗1o→0e) × √3, then scalar × vector. */
        double mjr_tp = tp_dot_(tp, w, NULL, m_j, r_hat);
        double T1_got[3];
        for (int i = 0; i < 3; ++i) T1_got[i] = mjr_tp * m_i[i];
        ...
        double T4_got[3];
        for (int i = 0; i < 3; ++i) T4_got[i] = mij_tp * m_i[i];
    ...
    const char *forces =
        "tests/test_downstream_compat/torque_net_tp_paths/expected_forces.json";
