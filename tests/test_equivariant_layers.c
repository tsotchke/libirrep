...
int main(void) {
    ...
    /* -------- linear: unmatched labels contribute nothing ---------- */
    {
        ...
        for (int i = 1; i < 6; ++i) IRREP_ASSERT_NEAR(y[i], 0.0, tol);
    ...
    /* -------- linear: SO(3) equivariance on a matched multiset ------- */
    {
        ...
        for (int i = 0; i < 3; ++i)
            for (int j = 0; j < 3; ++j) {
                double _Complex s = 0;
                for (int k = 0; k < 3; ++k) s += U1[i*3+k] * D1[k*3+j];
                tmp[i*3+j] = s;
            }
        for (int i = 0; i < 3; ++i)
            for (int j = 0; j < 3; ++j) {
                double _Complex s = 0;
                for (int k = 0; k < 3; ++k) s += tmp[i*3+k] * conj(U1[j*3+k]);
                Dr1[i*3+j] = s;
            }
        ...
        for (int u = 0; u < 2; ++u) {
            ...
            for (int i = 0; i < 3; ++i) x_rot[1 + u*3 + i] = out3[i];
        }
        ...
        for (int u = 0; u < 2; ++u) {
            ...
            for (int i = 0; i < 3; ++i) y_out_rot[1 + u*3 + i] = out3[i];
        }
    ...
    /* -------- linear backward vs finite difference ---------- */
    {
        ...
        irrep_linear_t *lin = irrep_linear_build(in, out, 1, 1);
        int nw = irrep_linear_num_weights(lin);
        double W[10];  for (int i = 0; i < nw; ++i) W[i] = 0.3 * (i + 1);
        ...
        for (int i = 0; i < nw; ++i) {
            double save = W[i];
            double yp[4], yn[4];
            W[i] = save + h; irrep_linear_apply(lin, W, x, yp);
            W[i] = save - h; irrep_linear_apply(lin, W, x, yn);
            W[i] = save;
            double fd = 0; for (int k = 0; k < 4; ++k) fd += grad_out[k] * (yp[k] - yn[k]) / (2 * h);
            IRREP_ASSERT(fabs(grad_W[i] - fd) < 1e-6);
        }
        for (int i = 0; i < 5; ++i) {
            double save = x[i];
            double yp[4], yn[4];
            x[i] = save + h; irrep_linear_apply(lin, W, x, yp);
            x[i] = save - h; irrep_linear_apply(lin, W, x, yn);
            x[i] = save;
            double fd = 0; for (int k = 0; k < 4; ++k) fd += grad_out[k] * (yp[k] - yn[k]) / (2 * h);
    ...
    /* -------- RMS norm backward vs FD ---------- */
    {
        ...
        for (int i = 0; i < 3; ++i) {
            double save = in[i];
            double yp[3], yn[3];
            in[i] = save + h; irrep_norm_rms(M, 1, scales, in, yp);
            in[i] = save - h; irrep_norm_rms(M, 1, scales, in, yn);
            in[i] = save;
            double fd = 0; for (int k = 0; k < 3; ++k) fd += go[k] * (yp[k] - yn[k]) / (2 * h);
            IRREP_ASSERT(fabs(grad_in[i] - fd) < 1e-6);
        }
        double save = scales[0];
        double yp[3], yn[3];
        scales[0] = save + h; irrep_norm_rms(M, 1, scales, in, yp);
        scales[0] = save - h; irrep_norm_rms(M, 1, scales, in, yn);
        scales[0] = save;
        double fd_s = 0; for (int k = 0; k < 3; ++k) fd_s += go[k] * (yp[k] - yn[k]) / (2 * h);
    ...
    /* -------- gate: multiplies each block by its gate ---------- */
    {
        ...
        for (int i = 0; i < 3; ++i) IRREP_ASSERT_NEAR(out[i],      -in[i],        tol);
        for (int i = 0; i < 3; ++i) IRREP_ASSERT_NEAR(out[3 + i],  2.5 * in[3+i], tol);
        irrep_multiset_free(M);
    }
