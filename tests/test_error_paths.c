...
int main(void) {
    ...
    /* ---- NequIP build: bad params fail cleanly ---- */
    {
        ...
        IRREP_ASSERT(h != NULL);
        /* r_cut = 0 */
        IRREP_ASSERT(irrep_nequip_layer_build(h, 2, 4, 0.0, IRREP_NEQUIP_CUTOFF_POLYNOMIAL, 6, h) ==
                     NULL);
        /* n_radial = 0 */
        IRREP_ASSERT(irrep_nequip_layer_build(h, 2, 0, 3.0, IRREP_NEQUIP_CUTOFF_POLYNOMIAL, 6, h) ==
                     NULL);
        /* l_sh_max negative */
        IRREP_ASSERT(
            irrep_nequip_layer_build(h, -1, 4, 3.0, IRREP_NEQUIP_CUTOFF_POLYNOMIAL, 6, h) == NULL);
