...
int main(void) {
     ...
     * energy E_0 = −2J (Bethe-ansatz / exact result for N = 4).         */
    {
        ...
        double         eig[2] = {0, 0};
        irrep_status_t rc = irrep_lanczos_eigvals(irrep_heisenberg_apply, H, dim, 2, 60, seed, eig);
    ...
    /* ---- XY model: H = J(S^x_0 S^x_1 + S^y_0 S^y_1) = ½J(S^+ S^- + h.c.) ---- */
    {
        ...
        irrep_heisenberg_apply(psi, out, H);
        for (int k = 0; k < 4; ++k)
            IRREP_ASSERT_NEAR(creal(out[k]), 0.0, 1e-14);
     ...
     * recovers the pure-NN-Heisenberg result bit-exactly. */
    {
        ...
        irrep_heisenberg_t *Hj1j2_zero =
            irrep_heisenberg_j1j2_new(3, 1, nn_i, nn_j, 1.0, 2, nnn_i, nnn_j, 0.0);
        ...
        irrep_heisenberg_t *Hequal =
            irrep_heisenberg_j1j2_new(3, 1, nn_i, nn_j, 1.0, 2, nnn_i, nnn_j, 1.0);
        ...
        double         eig[1];
        irrep_status_t rc =
            irrep_lanczos_eigvals(irrep_heisenberg_apply, Hequal, dim, 1, 40, seed, eig);
        IRREP_ASSERT(rc == IRREP_OK);
