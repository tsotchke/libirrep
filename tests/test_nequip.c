...
static void build_real_rot_l_(int l, double *R_out, double alpha, double beta, double gamma) {
    int d = 2 * l + 1;
...
static void rotate_multiset_(const irrep_multiset_t *m, double alpha, double beta, double gamma,
...
int main(void) {
    ...
    irrep_nequip_layer_t *layer =
        irrep_nequip_layer_build(h_in, /*l_sh_max=*/2, /*n_radial=*/4, /*r_cut=*/3.0,
    ...
    for (int i = 0; i < n_nodes * h_in_dim; ++i)
        h_in_buf[i] = 0.17 * i - 0.3;
    for (int i = 0; i < nw; ++i)
        w[i] = 0.23 * i - 0.5;

    irrep_nequip_layer_apply(layer, w, n_nodes, n_edges, edge_src, edge_dst, edge_vec, h_in_buf,
                             h_out_buf);

    /* Output should be finite and at least the first dst node gets something. */
    for (int i = 0; i < n_nodes * h_out_dim; ++i)
        IRREP_ASSERT(isfinite(h_out_buf[i]));
    ...
    for (int i = 0; i < n_nodes; ++i) {
        rotate_multiset_(h_in, alpha, beta, gamma, h_in_buf + (size_t)i * h_in_dim,
    ...
    irrep_nequip_layer_apply(layer, w, n_nodes, n_edges, edge_src, edge_dst, edge_vec_rot, h_in_rot,
                             h_out_rot_in);

    /* Rotate the direct output. */
    double *h_out_rotated = calloc((size_t)n_nodes * h_out_dim, sizeof(double));
    for (int i = 0; i < n_nodes; ++i) {
        rotate_multiset_(h_out, alpha, beta, gamma, h_out_buf + (size_t)i * h_out_dim,
    ...
    /* -------- Backward pass: finite-difference cross-check on one node ------- */
    {
        ...
        for (int i = 0; i < n_nodes * h_out_dim; ++i)
            grad_h_out[i] = 0.11 * i + 0.2;

        irrep_nequip_layer_apply_backward(layer, w, n_nodes, n_edges, edge_src, edge_dst, edge_vec,
                                          h_in_buf, grad_h_out, grad_h_in, grad_w);
        ...
        /* FD on h_in[0][0]. */
        {
            ...
            h_in_buf[0] = save + h;
            irrep_nequip_layer_apply(layer, w, n_nodes, n_edges, edge_src, edge_dst, edge_vec,
                                     h_in_buf, cp);
            h_in_buf[0] = save - h;
            irrep_nequip_layer_apply(layer, w, n_nodes, n_edges, edge_src, edge_dst, edge_vec,
                                     h_in_buf, cm);
     ...
     * single-edge spot check. */
    {
        ...
        for (int i = 0; i < n_nodes * h_out_dim; ++i)
            grad_h_out[i] = 0.07 * i + 0.1;

        irrep_nequip_layer_apply_forces(layer, w, n_nodes, n_edges, edge_src, edge_dst, edge_vec,
        ...
        for (int e = 0; e < n_edges; ++e) {
            for (int axis = 0; axis < 3; ++axis) {
                ...
                irrep_nequip_layer_apply(layer, w, n_nodes, n_edges, edge_src, edge_dst, edge_fd,
                                         h_in_buf, hp);
                edge_fd[e * 3 + axis] = save - h;
                irrep_nequip_layer_apply(layer, w, n_nodes, n_edges, edge_src, edge_dst, edge_fd,
                                         h_in_buf, hm);
     ...
     */
    {
        ...
        for (size_t c = 0; c < sizeof(cases) / sizeof(cases[0]); ++c) {
            ...
            irrep_nequip_layer_t *lc =
                irrep_nequip_layer_build(hi, cases[c].l_sh, /*n_radial=*/3, /*r_cut=*/3.0,
            ...
            for (int i = 0; i < nn * hi->total_dim; ++i)
                hin[i] = 0.11 * i + 0.2;

            double *gho = calloc((size_t)nn * ho->total_dim, sizeof(double));
            for (int i = 0; i < nn * ho->total_dim; ++i)
                gho[i] = 0.09 * i - 0.1;
