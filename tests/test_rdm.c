...
static void apply_dense_(const double _Complex *x, double _Complex *y, void *ctx) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < n; ++j) acc += H[(size_t)i * n + j] * x[j];
        y[i] = acc;
    }
}
...
int main(void) {
    ...
    /* Kitaev-Preskill arithmetic                                          */
    /* ------------------------------------------------------------------ */
    double g = irrep_topological_entanglement_entropy(1.0, 1.0, 1.0,
                                                      2.0, 2.0, 2.0,
                                                      3.0);
    ...
    double gamma_ghz = irrep_topological_entanglement_entropy(
    ...
    double ev_lan[2];
    irrep_status_t rc = irrep_lanczos_eigvals(apply_dense_, NULL,
                                              (long long)n_lan,
                                              /*k_wanted=*/2,
                                              /*max_iters=*/n_lan,
                                              /*seed=*/NULL,
    ...
    free(H_lan); free(H_copy); free(ev_ref);

    /* ------------------------------------------------------------------ */
    /* Error paths                                                         */
    /* ------------------------------------------------------------------ */
    IRREP_ASSERT(irrep_partial_trace(0, 2, NULL, NULL, 0, NULL)
                 == IRREP_ERR_INVALID_ARG);
    IRREP_ASSERT(irrep_hermitian_eigvals(-1, NULL, NULL)
                 == IRREP_ERR_INVALID_ARG);
